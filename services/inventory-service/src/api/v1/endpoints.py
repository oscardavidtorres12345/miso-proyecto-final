...
def upsert_stock(payload: StockUpsertRequest) -> StockResponse:
    return inventory_service.upsert_stock(payload)


@router.post("/holds", response_model=HoldResponse, status_code=status.HTTP_201_CREATED)
def create_hold(payload: CreateHoldRequest) -> HoldResponse:
    try:
        return inventory_service.create_hold(payload)
...
def get_hold(hold_id: str) -> HoldResponse:
    try:
        return inventory_service.get_hold(hold_id)
...
def confirm_hold(hold_id: str) -> ConfirmHoldResponse:
    try:
        return inventory_service.confirm_hold(hold_id)
    except HoldNotFoundError as exc:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)
        ) from exc
...
def cancel_hold(hold_id: str, payload: CancelHoldRequest) -> CancelHoldResponse:
    _ = payload
    try:
        return inventory_service.cancel_hold(hold_id)
    except HoldNotFoundError as exc:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)
        ) from exc
...
def expire_holds() -> ExpireHoldsResponse:
    return ExpireHoldsResponse(expired_count=inventory_service.expire_holds())
