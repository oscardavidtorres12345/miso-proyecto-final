...
from src.domain.services.login_service import LoginUnauthorizedError, LoginValidationError
...
_LOGIN_OK = LoginResponse(
    ...
    user=LoginUserInfo(user_id=1, username="oscar", email="oscar@test.com", is_active=True),
    permissions=["search"],
...
_REG_OK = RegisterResponse(
    status="created", sprint=1, hu_id="HU001",
    user_id=42, username="oscar", email="oscar@test.com",
    role="GUEST", jurisdiction_id=1, message="Registered",
)

_VALID_LOGIN = {"email": "oscar@test.com", "password": "secret123"}
_VALID_REG = {
    "first_name": "Oscar", "last_name": "Torres",
    "email": "oscar@test.com", "document_type_id": 1,
    "document_id": "123", "jurisdiction_id": 1,
...
def test_privacy_notice_ok(client: TestClient) -> None:
    mock_jurisdiction = MagicMock(
        iso_code="CO", region_name="Colombia",
        applicable_regulation="Ley 1581",
        privacy_title="Política", privacy_content="Contenido",
        privacy_pdf_url=[], privacy_version="1.0",
        privacy_effective_at=None, privacy_contact_email="privacy@co.com",
    )
