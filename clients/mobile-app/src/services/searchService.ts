...
const BASE_URL = API_CONFIG.SEARCH_URL;

function appendArray(params: URLSearchParams, key: string, values?: Array<string | number>): void {
  if (!values || values.length === 0) return;
  values.forEach((value) => params.append(key, String(value)));
}

export async function getSearchFilters(query: SearchFiltersQuery): Promise<SearchFiltersResponse> {
  ...
  const response = await fetch(`${BASE_URL}/search/filters?${params.toString()}`, {
    method: 'GET',
  });
...
export async function getSearchProperties(
  ...
  const response = await fetch(`${BASE_URL}/search/properties?${params.toString()}`, {
    method: 'GET',
  });

  if (!response.ok) {
    const error = new Error('Failed to fetch properties.') as Error & { status?: number };
