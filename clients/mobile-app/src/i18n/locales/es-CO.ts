const esCO = {
  search: {
    destination: 'Destino',
    wherePlaceholder: '¿Adónde vas?',
    dates: 'Fechas',
    addDates: 'Agrega fechas',
    who: 'Quién',
    howManyPlaceholder: '¿Cuántos?',
    search: 'Buscar',
    editSearch: 'Editar búsqueda',
    noResults: 'Ups, parece que no hay resultados por mostrar :( Intenta de nuevo.',
    filter: 'Filtros',
    clearFilters: 'Limpiar filtros',
    nights_one: '{{count}} noche',
    nights_other: '{{count}} noches',
    adults_one: '{{count}} adulto',
    adults_other: '{{count}} adultos',
    comments_one: '{{count}} comentario',
    comments_other: '{{count}} comentarios',
    taxesAndCharges: 'Incluye impuestos y cargos',
    viewDetails: 'Ver detalles',
  },
  filters: {
    title: 'Filtros',
    services: 'Servicios',
    accommodationType: 'Tipo de alojamiento',
    meals: 'Alimentación',
    stars: 'Estrellas',
    searchIn: 'Busca por {{title}}',
    showMore: 'Ver más',
    showLess: 'Ver menos',
    price: 'Precio',
    min: 'Mín.',
    max: 'Máx.',
    cancel: 'Cancelar',
    apply: 'Aplicar',
    service: {
      parking: 'Estacionamiento',
      pool: 'Piscina',
      pets: 'Acepta mascotas',
      kids: 'Servicios para niños',
      bathtub: 'Bañera',
      restaurant: 'Restaurante',
      spa: 'Spa',
      gym: 'Gimnasio',
      wifi: 'WiFi gratuito',
      ac: 'Aire acondicionado',
    },
    accommodation: {
      hotel: 'Hoteles',
      house: 'Casas',
      cabin: 'Cabañas',
      hostel: 'Hostales',
      villa: 'Villas',
      resort: 'Resorts',
    },
    meal: {
      breakfast: 'Desayuno',
      buffet: 'Desayuno buffet',
      allinclusive: 'Todo incluido',
    },
  },
  guests: {
    adults: 'Adultos',
    adultsAge: 'Edad: 13 años o más',
    children: 'Niños',
    childrenAge: 'Edad: 0 a 12 años',
    rooms: 'Habitaciones',
    pets: 'Mascotas',
    guest_one: '{{count}} huésped',
    guest_other: '{{count}} huéspedes',
  },
  header: {
    home: 'Ir al inicio',
    cart: 'Carrito',
    menu: 'Menú',
    login: 'Login',
    myBookings: 'Mis reservas',
    logout: 'Cerrar sesión',
    selectCountry: 'Seleccionar país',
    countries: {
      co: 'Colombia',
      ar: 'Argentina',
      us: 'Estados Unidos',
    },
  },
  pagination: {
    prevPage: 'Página anterior',
    nextPage: 'Página siguiente',
    page: 'Página {{page}}',
  },
  counter: {
    decrease: 'Reducir {{label}}',
    increase: 'Aumentar {{label}}',
  },
  common: {
    close: 'Cerrar',
    imageUnavailable: 'Imagen no disponible',
  },
  accommodationDetail: {
    underConstructionTitle: 'Estamos trabajando en esto',
    underConstructionSubtitle: 'La página de detalles del alojamiento estará disponible próximamente.',
    backToSearch: 'Volver a resultados',
  },
  footer: {
    madeWithLove: 'Hecho con amor 💚',
  },
  hero: {
    subtitle: 'Descubre tus próximas vacaciones',
    title: 'La vida es corta y el mundo es gigante.',
    search: 'Buscar',
  },
  features: {
    secureBookings: {
      title: 'Reservas seguras',
      description: 'Tus pagos y datos están protegidos en todo momento.',
    },
    bestPrices: {
      title: 'Mejores precios',
      description: 'Comparamos tarifas para que siempre encuentres la mejor opción.',
    },
    flexibleCancellation: {
      title: 'Cancelación flexible',
      description: 'Cambia de planes sin estrés con reembolso disponible.',
    },
    payCurrency: {
      title: 'Paga en pesos colombianos',
      description: 'Ve los precios en COP, sin sorpresas al finalizar.',
    },
  },
  destinations: {
    label: 'Destinos populares',
    heading: 'Los favoritos de nuestros viajeros',
  },
  travel: {
    label: 'Explora el mundo',
    heading: 'Cada viaje es una historia que contar',
    description:
      'Encuentra alojamientos únicos en los destinos más increíbles. Desde montañas hasta playas, tu próxima aventura está a un clic de distancia.',
  },
  calendar: {
    monthNames: ['enero', 'febrero', 'marzo', 'abril', 'mayo', 'junio', 'julio', 'agosto', 'septiembre', 'octubre', 'noviembre', 'diciembre'],
    monthNamesShort: ['ene', 'feb', 'mar', 'abr', 'may', 'jun', 'jul', 'ago', 'sep', 'oct', 'nov', 'dic'],
    dayNames: ['domingo', 'lunes', 'martes', 'miércoles', 'jueves', 'viernes', 'sábado'],
    dayNamesShort: ['do', 'lu', 'ma', 'mi', 'ju', 'vi', 'sa'],
    today: 'hoy',
  },
  bookings: {
    myReservationsTitle: 'Mis reservas',
    pastTripsTitle: 'Mis viajes anteriores',
    switchToPast: 'Viajes anteriores',
    switchToCurrent: 'Reservas',
    emptyMessage: 'Ups, parece que no hay nada que ver aquí.',
    dates: 'Fechas',
    arrival: 'Llegada',
    departure: 'Salida',
    guests: 'Huéspedes',
    bookingIdLabel: 'Reserva ID',
    guestCount_one: '{{count}} huésped',
    guestCount_other: '{{count}} huéspedes',
    checkIn: 'Check-in con QR',
    checkInSuccess: 'Check in realizado exitosamente.',
    checkInError: 'No fue posible realizar el check in. Inténtalo de nuevo.',
    manualCheckInCta: 'Check-in manual',
    manualCheckInTitle: 'Check-in manual',
    manualDocumentType: 'Tipo de documento',
    manualDocumentNumber: 'Número de documento',
    manualContactHint: 'Correo',
    manualDocumentNumberPlaceholder: 'Ej: 1020304050',
    manualContactHintPlaceholder: 'correo@ejemplo.com',
    documentTypeCC: 'CC',
    documentTypePassport: 'Pasaporte',
    manualCheckInConfirm: 'Confirmar',
    manualCheckInRequired: 'Completa número de documento y contacto.',
    manualInvalidEmail: 'Ingresa un correo válido.',
    cameraPermissionRequired: 'Debes permitir acceso a la cámara para escanear el QR.',
    scanQrTitle: 'Escanear código QR',
    scanQrHint: 'Apunta la cámara al QR del hotel para completar el check in.',
    closeScanner: 'Cerrar escáner',
    cancelReservation: 'Cancelar reserva',
    cancelReservationModalTitle: 'Cancelación de su reserva',
    cancelReservationModalMessage: 'Si cancela ahora, aplicará la política de cancelación vigente para esta reserva.',
    cancelReservationModalDismiss: 'Cancelar',
    cancelReservationModalConfirm: 'Estoy seguro',
    cancelSuccess: 'Reserva cancelada exitosamente.',
    cancelError: 'Error al cancelar la reserva. Inténtalo de nuevo.',
    statusCancelled: 'Cancelada',
  },
  login: {
    title: 'Inicia sesión en tu cuenta',
    noAccount: '¿No tienes cuenta? ',
    register: 'Regístrate',
    emailLabel: 'Correo',
    passwordLabel: 'Contraseña',
    fieldRequired: 'Campo requerido',
    invalidEmail: 'Correo inválido',
    hidePassword: 'Ocultar contraseña',
    showPassword: 'Mostrar contraseña',
    submit: 'Ingresar',
    apiSuccess: '¡Sesión iniciada! Redirigiendo...',
    noPermission: 'No tienes permisos para acceder a la aplicación móvil.',
    apiError: 'Correo o contraseña incorrectos. Intenta de nuevo.',
    sessionExpired: 'Tu sesión ha expirado. Por favor inicia sesión nuevamente.',
  },
} as const;

export default esCO;
